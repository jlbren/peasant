...
database_path='/my_path/databases'
...
def filter_by_coverage(contig_file,val,type_reads,*read_files):
    ...
    if type_reads=='pe':       
        ...
        if check_for_file(output_path+'/temp/aln.sam.gz')==False:
            ...
            log('Error with BBMap. Filter by coverage not performed.',output_path+'/log.txt')
            return contig_file
...
def filter_contigs(contig_file,filter_params):
...
def run_peasant(assembly,contig_file,type_reads,reads,filter_params,output_path,database_name):
    ...
    if filter_params!='':
        # run filters
        contig_file=filter_contigs(contig_file,filter_params)
        num_contigs_after_filtering=get_num_seqs(contig_file)
        print('# contigs after filtration: '+str(num_contigs_after_filtering)+' contigs')
